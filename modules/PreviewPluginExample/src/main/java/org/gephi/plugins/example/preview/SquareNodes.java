...
/**
 * Extends and replaces default nodes renderer and implements square shaped
 * nodes.
 * <p>
 * By specifically extending a preview default renderer, this replaces it and
 * the old one is no longer available. A default renderer can be extended in
 * order to add more features to it, or simply replace it completely. Default
 * renderers are contained in <code>org.gephi.preview.plugin.renderers</code>
 * package.
 * <p>
 * Note that we need to set the position parameter even this replaces other
 * renderer. In this case and normally, the position has been set the same that
 * the replaced renderer has (300).
 *
    ...
    public void render(Item item, RenderTarget target, PreviewProperties properties) {
        if (properties.getBooleanValue("SquareNodes.property.enable")) {
            if (target instanceof G2DTarget) {
                renderSquaresJava2D(item, (G2DTarget) target, properties);
