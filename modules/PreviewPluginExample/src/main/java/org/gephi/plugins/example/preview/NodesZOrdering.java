 ...
 * <p>
 * Nodes can overlap each other and it's nice to be able to decide in which
 * order nodes should be displayed. This example shows how to customize the
 * output of an existing {@link ItemBuilder}. Builders are responsible for
 * creating items (in this case <code>NodeItem</code>) and return an array of
 * items. This class replaces and extends the default {@link NodeBuilder}. It
 * simply sorts the array of items coming out of the <code>NodeBuilder</code>
 * using a <code>sort.column</code> parameter set in the
