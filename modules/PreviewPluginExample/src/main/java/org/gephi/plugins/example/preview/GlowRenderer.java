 ...
 * <p>
 * This example is rendering something new for an existing object (i.e. node).
 * That's why the <code>isRendererForitem()</code> returns <code>true</code> for
 * node items and the <code>render()</code> method will receive node items.
 * <p>
 * The rendering part shows how to retrieve attributes from node items such as
 * the position, the size and the color. Then it shows how to draw something
 * using Java2D. The PDF and SVG renderers are not implemented yet but doing a
 * radial gradient should be easy.
 * <p>
 ...
 * this renderer first. This is important as we want the glow to be under the
 * node.
 *
