...
/**
 * Filter that keeps the top N nodes. Nodes are sorted using an attribute
 * column.
 * <p>
 * This example shows how to use the <code>init()</code> method to initialize a
 * filter and how to deal with filter properties. The <code>init()</code> is
 ...
 * filter. Each property should have a name and the proper getters and setters.
 * For example if your filter has a 'top' of the type <code>Integer</code> the
 * class must have the <code>getTop()</code> and <code>setTop()</code>
 * accessors. Make sure to use <code>Integer</code>, <code>Double</code> etc.
 * and not the default types.
 *
 * @see TopNodesBuilder
