...
/**
 * AttributeColumnsManipulator that replaces a column with its dynamic
 * equivalent with a defined interval.
 *
...
public class ConvertColumnToDynamic implements AttributeColumnsManipulator {
    ...
    @Override
    public boolean canManipulateColumn(Table at, Column c) {
        if (AttributeUtils.isNodeColumn(c)) {
            if (!c.isProperty()) {
                //Can't replace id or label column
                return AttributeUtils.getIntervalMapType(c.getTypeClass()) != null;//Is convertible to some dynamic type?
            } else {
                return false;
            }
        } else if (AttributeUtils.isEdgeColumn(c)) {
            if (!c.isProperty()) {
                //Can't replace id, label or weight column
                return AttributeUtils.getIntervalMapType(c.getTypeClass()) != null;//Is convertible to some dynamic type?
            } else {
                return false;
            }
    ...
    @Override
    public AttributeColumnsManipulatorUI getUI(Table at, Column ac) {
