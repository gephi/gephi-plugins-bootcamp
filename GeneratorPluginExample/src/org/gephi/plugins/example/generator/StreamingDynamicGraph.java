...
/**
 * Random streaming dynamic graph generator. <p> Example of a generator not
 * using the container structure and directly pushing to the graph structure. It
 * shows how to create a graph from scratch and assign a time interval attribute
 * to nodes. <p> The generator creates a thread that will survive after the
 * generator method ends. It is controlled by a cancel flag that becomes
 * <code>true</code> if the user cancels the task. <p> This example directly
 * appends nodes and edges to the graph. A traditional generator go through the
 * container structure for adding objects. However as using the container can
 * only be done in the scope of the generator it can't be used for continuous
 * generators. Because this generator doesn't use the container it makes sure a
 * project and workspace is opened before starting to append data.
 *
